...
public class Wagon<E> {
	...
	public boolean pack(Intent intent, Class<? extends Object> objTypeToPack, Object instance, boolean packAllFields, String crateKey) {
		...
		for (Field field : declaredFields) {
			Annotation annotation = field.getAnnotation(james.a.grant.wagon.WoodBox.class);
			if (annotation == null)
			...
			if (isAPackableField(annotation) || packAllFields) {
				...
				} else if (annotation instanceof james.a.grant.wagon.WoodBox || packAllFields) {
	...
	private boolean isAPackableField(Annotation annotation) {
		if (annotation == null)
			return false;
		return annotation instanceof james.a.grant.wagon.WoodBox || annotation instanceof Crate;
	}
		...
		if (extras != null) {
			...
			for (Field field : declaredFields) {
				...
				if (isAPackableField(annotation) || unpackAllFields) {
					...
					} else if (annotation instanceof james.a.grant.wagon.WoodBox || unpackAllFields) {
	...
	private String getKey(Annotation annotation) {
		String key = "";
		if (annotation instanceof james.a.grant.wagon.WoodBox)
			key = ((james.a.grant.wagon.WoodBox) annotation).key();
		if (annotation instanceof Crate)
	...
	private boolean isPackablePreference(int level, Annotation annotation) {
		...
		if (level == 0) {
			if (annotation != null) {
				...
				} else if (annotation instanceof james.a.grant.wagon.WoodBox) {
					shouldPackPreference = ((james.a.grant.wagon.WoodBox) annotation).preference();
				}
			}
		} else {
